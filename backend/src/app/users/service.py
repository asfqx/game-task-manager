...
class UserService:
    ...
    @classmethod
    async def _serialize_user_profile(
        cls,
        target_user: User,
        viewer: User,
        ...
        visible_teams: list[UserTeamSummaryResponse] = []
        ...
        for team in teams:
            if (
                ...
                and not any(member.user_uuid == viewer.uuid for member in team.members)
            ):
                continue
            ...
            visible_teams.append(
                UserTeamSummaryResponse(
                    ...
                    lvl=LvlSummaryResponse(
                        uuid=membership.lvl.uuid,
                        value=membership.lvl.value,
                        required_xp=membership.lvl.required_xp,
                    ),
                ),
            )
    ...
    async def get_user_profile(
        ...
        return await cls._serialize_user_profile(exist_user, current_user, session)
    ...
    async def get_all(
        ...
        payload: list[GetUserProfileResponse] = []

        for user in users:
            payload.append(
                await cls._serialize_user_profile(user, current_user, session),
            )
    ...
    async def update_bio(
        ...
        await SystemLoggingService.log_user_action(
            ...
            entity_uuid=exist_user.uuid,
            details={
                "changed_fields": sorted(new_bio.model_dump(exclude_unset=True).keys()),
            },
        )
